...
enum {
    axi_tui_PROMPT_CAPACITY = 512,
    axi_tui_HISTORY_CAPACITY = 32,
    axi_tui_SCROLLBACK_CAPACITY = 128,
    axi_tui_LINE_CAPACITY = 512
};

struct AxiTerminal {
    char prompt[axi_tui_PROMPT_CAPACITY];
    char workspace[MAX_PATH];
    char history[axi_tui_HISTORY_CAPACITY][axi_tui_PROMPT_CAPACITY];
    size_t history_count;
    size_t history_cursor;
    char lines[axi_tui_SCROLLBACK_CAPACITY][axi_tui_LINE_CAPACITY];
    size_t line_count;
};
...
static void record_history(AxiTerminal *terminal, const char *command) {
    if (terminal->history_count == axi_tui_HISTORY_CAPACITY) {
        memmove(terminal->history, terminal->history + 1,
                (axi_tui_HISTORY_CAPACITY - 1) * axi_tui_PROMPT_CAPACITY);
        terminal->history_count--;
    }
    copy_text(terminal->history[terminal->history_count], axi_tui_PROMPT_CAPACITY, command);
    terminal->history_count++;
...
AxiTerminal *axi_tui_create(void) {
    AxiTerminal *terminal = (AxiTerminal *)calloc(1, sizeof(*terminal));
    return terminal;
}

void axi_tui_destroy(AxiTerminal *terminal) {
    free(terminal);
}

void axi_tui_insert_text(AxiTerminal *terminal, const char *text) {
    size_t existing;
    size_t available;
    size_t length;
    if (!terminal || !text) return;
    existing = strlen(terminal->prompt);
    available = axi_tui_PROMPT_CAPACITY - existing - 1;
    length = strlen(text);
...
void axi_tui_backspace(AxiTerminal *terminal) {
    size_t length;
    if (!terminal) return;
...
void axi_tui_history_previous(AxiTerminal *terminal) {
    if (!terminal || !terminal->history_count) return;
    if (terminal->history_cursor) terminal->history_cursor--;
    copy_text(terminal->prompt, axi_tui_PROMPT_CAPACITY,
              terminal->history[terminal->history_cursor]);
}

void axi_tui_history_next(AxiTerminal *terminal) {
    if (!terminal || !terminal->history_count) return;
...
int axi_tui_submit(AxiTerminal *terminal) {
    const char *command;
    char command_copy[axi_tui_PROMPT_CAPACITY];
    char *tokens[8];
    size_t token_count;
    char compiler[MAX_PATH];
    char command_word[axi_tui_LINE_CAPACITY];
    char unknown[axi_tui_LINE_CAPACITY];
    size_t length = 0;
...
const char *axi_tui_prompt(const AxiTerminal *terminal) {
    return terminal ? terminal->prompt : "";
}

size_t axi_tui_line_count(const AxiTerminal *terminal) {
    return terminal ? terminal->line_count : 0;
}

const char *axi_tui_line_at(const AxiTerminal *terminal, size_t index) {
    if (!terminal || index >= terminal->line_count) return "";
    return terminal->lines[index];
}

void tui_run_loop(void) {
    AxiTerminal *terminal = axi_tui_create();
    char input_buffer[1024];
    
    printf("Axi Native Terminal (FOSS CLI Edition)\n");
    printf("Type 'axi help' to list commands, or 'exit' to quit.\n\n");
    ...
    while (1) {
        ...
        if (strlen(input_buffer) > 0) {
            copy_text(terminal->prompt, sizeof(terminal->prompt), input_buffer);
            axi_tui_submit(terminal);
        }
    }
    
    axi_tui_destroy(terminal);
}
